...
export function validateTokenRushLevel(document) {
  ...
  assertSupported(spawn, 'SPAWN_SUPPORT');
  ...
  const enemies = document.enemies.map((value) => {
    ...
    assertSupported(position, 'ENEMY_SUPPORT');
    const key = `${position.x},${position.y}`;
    if (entityCells.has(key)) reject('ENEMY_OVERLAP');
    entityCells.add(key);
...
export function compileTokenRushLevel(document) {
  ...
  return deepFreeze({
    ...
    tiles,
    spawn: actorPosition(level.spawn, PLAYER_HALF_HEIGHT),
    exit: {
    ...
    enemies: level.enemies.map((enemy, index) => ({
      ...
      position: actorPosition(enemy, ENEMY_HALF_HEIGHT),
    })),
